...
def add_glass_background():
    ...
    if bg_b64:
        ...
        <style>
        .stApp {{
            background: url('data:image/jpeg;base64,{bg_b64}') center/cover no-repeat fixed !important;
        }}
        .stApp::before {{
            content:'';
            position:fixed;
...
LAYOUT = dict(
    ...
    legend=dict(bgcolor='rgba(20,20,20,0.7)', bordercolor='rgba(224,58,62,0.2)',
...
with tab2:
    ...
    with c2:
        ...
        fig2.update_layout(**LAYOUT, title='Top 10 — Value Score Ranking', height=400,
    ...
    fig3.update_layout(**LAYOUT, height=400, coloraxis_colorbar=dict(title='Score',tickfont=dict(color='#666'),bgcolor='rgba(20,20,20,0.6)'))
    ...
    if 'Defense_Hardness' in df.columns:
        ...
        fig4.update_layout(**LAYOUT, title='Defense Hardness (higher = harder to score against)', height=400,
    ...
    if selected:
        ...
        if len(selected) > 1:
            ...
            with cc1:
                ...
                fig_ga.update_layout(**LAYOUT, barmode='group', title='Goals & Assists', height=320,
...
with tab4:
    ...
    with ci1:
        ...
        fig_p.update_layout(**LAYOUT, title='Position Distribution', height=300)
        st.plotly_chart(fig_p, use_container_width=True)
    with ci2:
        fig_a = go.Figure(go.Histogram(x=df['Age_num'], nbinsx=15,
            marker_color='#e03a3e', opacity=0.7))
        fig_a.update_layout(**LAYOUT, title='Age Distribution', height=300,
